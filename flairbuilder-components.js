(function() {
    ...
    CheckBox.prototype.attachTriggers = function() {
      ...
      return checkbox.on('click', $.proxy(function(event) {
        ...
        if (checkbox.is(':checked')) {
          this.dispatchEvent(CHECKBOX_SELECTED);
        } else {
          this.dispatchEvent(CHECKBOX_UNSELECTED);
        }
        return false;
      }, this));
    };
  ...
  TabNavigator = (function(_super) {
    ...
    TabNavigator.prototype.doRender = function(e) {
      var after, content, horizontal, l, label, lis, self, tab, tablist, _i, _len, _ref;
      this.labels = [];
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        label = new fb.LabeledObject(tab, this);
        this.labels.push(label);
        tablist.append('<li><div>' + label.htmlText + '</div></li>');
      }
      tablist.addClass('tabnavigator-' + this.position);
      tablist.addClass('tabnavigator-tabs');
      if (!after) {
        e.append(content);
      }
      e.append(tablist);
      ...
      if (horizontal) {
        ...
        if (this.position.indexOf("-middle") > -1) {
          tablist.css("padding", "" + (this.height - this.labels.length * this.tabHeight - (this.labels.length + 1) * this.borderThickness) / 2 + "px 0");
        } else if (this.position.indexOf("-top") > -1) {
          tablist.css("padding", "0");
        } else if (this.position.indexOf("-bottom") > -1) {
          tablist.css("padding", "" + (this.height - this.labels.length * this.tabHeight - (this.labels.length + 1) * this.borderThickness) + "px 0 0 0");
        }
        tablist.css("width", this.tabWidth - 1).css("height", this.height - 1);
        content.css("width", this.width - this.tabWidth).css("height", this.height);
        if (after) {
          content.css("left", l);
        } else {
          tablist.css("left", this.width - l);
        }
      } else {
        l = 1 * this.tabHeight + 1 * this.borderThickness;
        if (this.position.indexOf("-center") > -1) {
          tablist.css("padding", "0 " + (this.width - this.labels.length * this.tabWidth - (this.labels.length + 1) * this.borderThickness) / 2 + "px");
        } else if (this.position.indexOf("-left") > -1) {
          tablist.css("padding", "0");
        } else if (this.position.indexOf("-right") > -1) {
          tablist.css("padding", "0 0 0 " + (this.width - this.labels.length * this.tabWidth - (this.labels.length + 1) * this.borderThickness) + "px");
        }
        tablist.css("width", this.width).css("height", this.tabHeight);
        content.css("width", this.width + this.borderThickness * 1).css("height", this.height - this.tabHeight);
        if (after) {
          content.css("top", l);
        } else {
          tablist.css("top", this.height - l);
        }
      }
      lis = tablist.find('li');
      ...
      tablist.find('li > div').css('text-align', this.textAlign);
      self = this;
      tablist.find('li > div').on('click', function(event) {
        var index;
        index = $(this).parent().index();
        return self.showTab(index);
      });
